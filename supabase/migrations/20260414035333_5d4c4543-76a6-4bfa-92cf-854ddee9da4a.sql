...
BEGIN
  ...
  FROM (
    ...
    FROM regexp_split_to_table(
      regexp_replace(
        regexp_replace(
          regexp_replace(
            regexp_replace(v_primary_name_ar, '[ً-ٰٟ]', '', 'g'),
            '[إأآٱ]', 'ا', 'g'
          ),
          'ة', 'ه', 'g'
        ),
  ...
  IF v_library_id IS NULL THEN
    ...
    VALUES (
      coalesce(v_primary_name_ar, ''),
      coalesce(v_primary_name_en, ''),
      nullif(v_full_description_ar, ''),
      v_detected_category,
      v_item.unit,
      p_unit_rate,
  ...
  ELSE
    ...
    SET
      ...
      standard_name_en = CASE
        WHEN nullif(v_primary_name_en, '') IS NOT NULL THEN v_primary_name_en
        ELSE standard_name_en
      END,
      item_description = CASE
        WHEN nullif(v_full_description_ar, '') IS NOT NULL THEN v_full_description_ar
      ...
      item_name_aliases = ARRAY(
        ...
        FROM unnest(coalesce(item_name_aliases, '{}'::text[]) || v_aliases) alias
  ...
  WHERE bi.unit = v_item.unit
    AND bi.id <> p_item_id
    AND (
      lower(coalesce(bi.description, '')) = lower(v_primary_name_ar)
      OR lower(coalesce(bi.description, '')) LIKE '%' || lower(v_primary_name_ar) || '%'
      OR lower(v_primary_name_ar) LIKE '%' || lower(coalesce(bi.description, '')) || '%'
      OR lower(coalesce(bi.description, '')) LIKE '%' || lower(v_full_description_ar) || '%'
    );
...
UPDATE public.rate_library
SET standard_name_ar = btrim(regexp_replace(standard_name_ar, '^.*[—-]\s*', '')),
    standard_name_en = CASE
      WHEN coalesce(standard_name_en, '') = '' THEN standard_name_en
      ELSE btrim(regexp_replace(standard_name_en, '^.*[—-]\s*', ''))
    END,
    item_description = COALESCE(NULLIF(item_description, ''), standard_name_ar),
    category = 'steel_misc',
    item_name_aliases = ARRAY(
      SELECT DISTINCT alias
