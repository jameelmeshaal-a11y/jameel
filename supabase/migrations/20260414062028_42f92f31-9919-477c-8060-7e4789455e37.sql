...
-- 1) Helper: extract sub-item name (text after last — or -)
...
-- 2) Fix save_manual_price: replace the broad LIKE matching at the end
CREATE OR REPLACE FUNCTION public.save_manual_price(p_item_id uuid, p_boq_file_id uuid, p_materials numeric, p_labor numeric, p_equipment numeric, p_logistics numeric, p_risk numeric, p_profit numeric, p_unit_rate numeric, p_total_price numeric, p_manual_overrides jsonb DEFAULT '{}'::jsonb, p_correction_note text DEFAULT NULL::text, p_user_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Extract the sub-item name for precise matching
  ...
  WHERE rl.unit = v_item.unit
    ...
    AND (
      ...
      OR EXISTS (
        SELECT 1
        FROM unnest(coalesce(rl.item_name_aliases, '{}'::text[])) alias
        WHERE lower(v_primary_name_ar) LIKE '%' || lower(alias) || '%'
           OR lower(alias) LIKE '%' || lower(v_primary_name_ar) || '%'
           OR lower(v_full_description_ar) LIKE '%' || lower(alias) || '%'
      )
    )
  ORDER BY
  ...
  -- *** FIXED: Precise matching for similar items ***
  -- Only link items that have the EXACT SAME sub-item name AND same unit
  -- This prevents cross-linking between columns, beams, slabs, etc.
...
-- 3) Data fix: Unlink items wrongly linked to ground slab record 0c7296bf
-- Only items whose sub-item name is NOT the ground slab should be unlinked
UPDATE public.boq_items
SET
  linked_rate_id = NULL,
  status = CASE
    WHEN override_type = 'manual' THEN status
    ELSE 'pending'
  END
WHERE linked_rate_id = '0c7296bf-3f42-4ec2-9e25-78bdf22af18d'
  AND id NOT IN (
    -- Keep items that ARE actually ground slabs
    SELECT id FROM public.boq_items
    WHERE linked_rate_id = '0c7296bf-3f42-4ec2-9e25-78bdf22af18d'
      AND (
        public.extract_sub_item(description) ~* '(بلاطة.*أرض|بلاطه.*ارض|ground.*slab|slab.*on.*grade|بلاطة على الأرض)'
