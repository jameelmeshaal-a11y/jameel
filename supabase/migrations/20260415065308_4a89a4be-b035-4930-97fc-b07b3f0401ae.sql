...
BEGIN
  ...
  -- *** Propagation 2: Same library item, same BoQ file + HARDENED SIMILARITY GATE ***
  -- GOVERNANCE: similarity >= 0.85 AND exact unit match AND same file
  UPDATE public.boq_items bi
  SET
    unit_rate = p_unit_rate,
    total_price = round(p_unit_rate * bi.quantity, 2),
  ...
  WHERE bi.linked_rate_id = v_library_id
    ...
    AND (bi.override_type IS NULL OR bi.override_type != 'manual')
    AND public.similarity(public.extract_sub_item(bi.description), v_sub_item_name) > 0.85;
