...
export async function runPricingEngine(
  ...
  // ── BMS Points-Based Pricing — DISABLED (V4.4 strict library-only) ────
  // Library is the SOLE source of pricing. BMS auto-pricing removed because
  // it generates prices outside the rate_library, violating governance.
...
export async function repriceSingleItem(
  ...
  // ── BMS Detection: DISABLED (V4.4 strict library-only) ──
  // BMS items now go through the standard library-match flow like every other item.
